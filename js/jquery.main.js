...
$(document).ready(function(){
	...
	$('.side-box').hover(
		...
		function(){
			$(this).find('.text').animate({marginLeft: '-150px'}, 500)
		}
	)
	$(function() {
		$( "#tabs" ).tabs({event: "hover"});
	})
	...
	$('.gallery2 .list li a').click(function(){
		var box = $('.gallery2 .switcher'),
			txt = $(this).find('.txt').html(),
			img = $(this).find('img').attr('src')
		console.log(txt)
		if (txt){
			console.log($(box).find('.text-box p').html)
			$(box).find('.text-box').css('visibility','visible')
			$(box).find('.text-box p').html(txt) 
		} else {
			$(box).find('.text-box').css('visibility','hidden')
		}
